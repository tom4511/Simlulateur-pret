...
DEPENSES_TEMPLATE = {
    ...
    "Transports": [
        ...
        ("Transports en commun", 40, "Mensuelle"),
        ("Entretien auto & pneus", 400, "Annuelle"),
    ],
    "Abonnements & Services": [
        ("Box Internet & Téléphones", 60, "Mensuelle"),
        ("Streaming & médias", 30, "Mensuelle"),
        ("Sport, clubs & loisirs", 45, "Mensuelle"),
    ],
    "Assurances & Taxes hors immo": [
        ("Assurance habitation", 240, "Annuelle"),
        ("Mutuelle santé", 40, "Mensuelle"),
    ],
    "Enfants & Famille": [
        ("Frais de garde", 0, "Mensuelle"),
...
DEFAULT_PARAMS = {
    ...
    "ancien_pret_total_emprunte": 200000.0,
    "ancien_pret_deja_rembourse": 50000.0,
...
def calcul_capacite(mensualite, taux_annuel, duree):
    ...
    return mensualite * ((1 - (1 + taux_mensuel) ** (-n)) / taux_mensuel)


def tableau_amortissement_pret(nom_pret, capital, taux_annuel, duree):
    if capital <= 0 or duree <= 0:
        return pd.DataFrame(columns=["Mois", "Prêt", "Mensualité", "Capital remboursé", "Intérêts", "Capital restant dû"])

    taux_mensuel = taux_annuel / 100 / 12
    nb_mois = int(duree * 12)
    mensualite = calcul_mensualite(capital, taux_annuel, duree)
...
def depense_key(name):
    return name.lower().replace(" ", "_").replace("/", "_").replace("&", "et").replace("-", "_")
...
st.set_page_config(page_title="Dashboard fusionné prêt + reste à vivre", page_icon="🏠", layout="wide")
st.title("🏠 Dashboard fusionné : Simulateur de prêt + Reste à vivre")

params = charger_params()

st.sidebar.title("Paramètres partagés")
...
charges_fixes = st.sidebar.number_input("Charges fixes hors dépenses détaillées (€/mois)", min_value=0.0, value=float(params["charges_fixes"]), step=50.0)
...
taux_endettement = st.sidebar.slider("Taux d'endettement (%)", 20, 50, int(max(20, min(50, params["taux_endettement"]))), disabled=mode_simulation == "Mensualité")
...
if activer_pret_relais:
    ancien_pret_total_emprunte = st.sidebar.number_input("Ancien prêt - Total emprunté (€)", min_value=0.0, value=float(params["ancien_pret_total_emprunte"]))
    ancien_pret_deja_rembourse = st.sidebar.number_input("Ancien prêt - Déjà remboursé (€)", min_value=0.0, value=float(params["ancien_pret_deja_rembourse"]))
    prix_vente_estime = st.sidebar.number_input("Prix de vente estimé (€)", min_value=0.0, value=float(params["prix_vente_estime"]))
    facteur_revente_banque_pct = st.sidebar.number_input("Facteur de revente banque (%)", min_value=0.0, max_value=100.0, value=float(params["facteur_revente_banque_pct"]))
else:
    ancien_pret_total_emprunte = float(params["ancien_pret_total_emprunte"])
    ancien_pret_deja_rembourse = float(params["ancien_pret_deja_rembourse"])
...
montant_total_a_financer = max(prix_bien + frais_notaire - apport_total, 0)

capital_comp_1 = max(pret_comp_1_montant, 0)
capital_comp_2 = max(pret_comp_2_montant, 0)
...
if mode_simulation == "Mensualité":
    capital_principal = max(montant_total_a_financer - capital_complementaires, 0)
    ...
    mensualite_max = revenu_total * (taux_endettement / 100) - charges_fixes
    prix_max = None
else:
    mensualite_max = revenu_total * (taux_endettement / 100) - charges_fixes
    mensualite_dispo_principal = max(mensualite_max - mensualite_comp_1 - mensualite_comp_2, 0)
    capital_principal = calcul_capacite(mensualite_dispo_principal, taux_credit, duree_principale)
    assurance_mensuelle = capital_principal * (taux_assurance / 100) / 12
    mensualite_principal = calcul_mensualite(capital_principal, taux_credit, duree_principale)
    mensualite_totale = mensualite_principal + mensualite_comp_1 + mensualite_comp_2 + assurance_mensuelle
    prix_max = (capital_principal + capital_complementaires + apport_total) / (1 + frais_notaire_pct / 100) if (1 + frais_notaire_pct / 100) > 0 else 0
...
onglet_pret, onglet_rav = st.tabs(["📈 Simulateur de prêt", "💶 Simulateur reste à vivre"])
...
with onglet_pret:
    ...
    if mode_simulation == "Mensualité":
        ...
        col3.metric("Prix d'achat possible", f"{prix_max:,.0f} €")

    st.caption(
        f"Apport personnel: {apport:,.0f} € | Apport additionnel relais: {apport_additionnel_relais:,.0f} € | Apport total: {apport_total:,.0f} €"
    )

    if mode_simulation == "Capacité d'emprunt":
        st.caption(
            f"Mensualité max: {mensualite_max:,.0f} € | Mensualités complémentaires: {(mensualite_comp_1 + mensualite_comp_2):,.0f} €"
        )

    st.subheader("Tableau d'amortissement agrégé")
    st.dataframe(df_amort, use_container_width=True, height=420)

    if not df_amort.empty:
        fig_crd = px.line(df_amort, x="Mois", y="Capital restant dû", title="Évolution du capital restant dû")
        st.plotly_chart(fig_crd, use_container_width=True)

    if not df_amort_detail.empty:
        st.subheader("Détail des prêts")
        fig_area = px.area(
            df_amort_detail.pivot_table(index="Mois", columns="Prêt", values="Capital restant dû", aggfunc="sum").reset_index(),
            x="Mois",
            y=[col for col in df_amort_detail["Prêt"].unique()],
            title="Capital restant dû par prêt",
        )
...
with onglet_rav:
    ...
    k2.metric("Mensualité prêt", f"{mensualite_totale:,.0f} €/mois", delta=f"Taux: {taux_endettement_reel:.1f}%")
    ...
    if taux_endettement_reel > 35:
        st.warning(f"Taux d'endettement élevé: {taux_endettement_reel:.1f}%")
    if epargne_disponible < 0:
        st.error(f"Épargne négative de {abs(epargne_disponible):,.0f} €/mois")
    ...
    with c1:
        ...
        fig_global = px.pie(repartition, values="Montant", names="Poste", hole=0.45, title="Répartition du revenu")
    ...
    st.subheader("Flux mensuel")
    waterfall_x = ["Revenus", "Prêt", "Charges fixes"]
